...
class SearchController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($exactStore && $request->ajax()) {
            return response()->json([
                'status' => 'redirect',
                'url' => route('employee.store.show', $exactStore->id)
            ]);
        }
        ...
        if ($request->ajax()) {
            ...
            if ($stores->count() > 0) {
                $html .= view('employee.search.partials.stores', ['stores' => $stores])->render();
            }
            
            if ($coupons->count() > 0) {
                $html .= view('employee.search.partials.coupons', ['coupons' => $coupons])->render();
            }
            
            if ($blogs->count() > 0) {
                $html .= view('employee.search.partials.blogs', ['blogs' => $blogs])->render();
            }
            
            if ($categories->count() > 0) {
                $html .= view('employee.search.partials.categories', ['categories' => $categories])->render();
            }
            
            if ($networks->count() > 0) {
                $html .= view('employee.search.partials.networks', ['networks' => $networks])->render();
            }
        ...
        return redirect()->route('employee.search.index', ['query' => $query]);
    }
    ...
    public function searchResults(Request $request)
    {
        ...
        if (empty($query)) {
            return redirect()->route('employee.store.index')->with('error', 'Please enter a search term');
        }
        ...
        if ($exactStore) {
            return redirect()->route('employee.store.show', $exactStore->id);
        }

        return view('employee.search.index', [
            'query' => $query,
            'stores' => $stores,
            'coupons' => $coupons,
            'blogs' => $blogs,
    ...
    public function autocomplete(Request $request)
    {
        ...
        foreach ($stores as $store) {
            $results[] = [
                ...
                'slug' => $store->slug,
                'image' => $store->image,
                'url' => route('employee.store.show', $store->id),
