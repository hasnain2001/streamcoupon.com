...
@section('content')
    <div class="error-container">
        ...
        {{-- Error Code with speedometer icon --}}
        ...
        <p class="error-message">
            {{ __('You have sent too many requests in a short period. Please wait a moment and try again.') }}
        </p>

        {{-- Action Buttons --}}
        <div class="action-buttons">
            <a href="{{ url()->current() }}" class="btn-home" onclick="event.preventDefault(); location.reload();">
                <i class="bi bi-arrow-repeat"></i> Try Again
            </a>
            <a href="{{ url('/') }}" class="btn-outline-help">
        ...
        {{-- Optional: show retry-after header if available --}}
        @if ($exception && $exception->getHeaders() && isset($exception->getHeaders()['Retry-After']))
            <p class="retry-note mt-3">
                <i class="bi bi-clock me-1"></i>
                {{ __('Please wait :seconds seconds before trying again.', ['seconds' => $exception->getHeaders()['Retry-After']]) }}
            </p>
        @endif
    </div>
    ...
    @push('styles')
        <style>
            ...
            .error-code i {
                font-size: 0.65em;
                -webkit-text-fill-color: #f97316; /* orange for rate limit */
                color: #f97316;
                background: none;
            }
            .error-code span {
            ...
            /* Retry note (if Retry-After header is present) */
            .retry-note {
                font-size: 0.9rem;
                color: #64748b;
                position: relative;
